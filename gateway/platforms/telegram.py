...
class TelegramAdapter(BasePlatformAdapter):
    ...
    def _thread_kwargs_for_send(
        ...
        topic thread id together with a reply anchor unless explicit Bot API
        DM-topic metadata is present. Lifecycle/resumed sends must preserve
        ``direct_messages_topic_id`` so they land in the visible DM topic;
        ``message_thread_id`` alone can render outside the visible lane.

        When ``reply_to_mode`` is ``"off"``, the reply anchor is suppressed for
        DM topic fallback sends. Explicit Bot API DM-topic metadata remains
        authoritative; legacy fallback metadata without it preserves the
        ``message_thread_id`` route.
        """
        if metadata and metadata.get("telegram_dm_topic_reply_fallback"):
